...
description: "Use only when the user explicitly requests Agent Workspace, an isolated/hidden workspace, a workspace-owned browser, a workspace or profile ID, sandboxed GUI observation, or stale workspace cleanup."
---
...
### Activate only for the isolated workspace surface
This skill is selected by an explicit request for **Agent Workspace**, a
**hidden workspace**, an **isolated workspace**, a workspace-owned browser, or
a workspace/profile identifier. It is not a generic Linux computer-use skill.

If the user says **Linux Computer Use**, **host desktop**, **real desktop**,
**my desktop**, or asks to operate their currently open host windows, do not use
this skill or any `agent-workspace-linux` tools. Use the host Linux Computer Use
surface instead. Likewise, explicit in-app Browser or Chrome Control requests
remain on those named browser surfaces.

Once this skill is selected, keep desktop interaction inside the isolated
workspace. If the workspace backend is unavailable or fails, report the exact
failure rather than switching to a host surface.
